...
func (r BasketRepository) Find(ctx context.Context, basketID string) (*domain.Basket, error) {
	const query = "SELECT customer_id, payment_id, items, sttaus FROM %s WHERE id = $1 LIMIT 1"
	...
	err := r.db.QueryRowContext(ctx, r.table(query), basketID).
		Scan(&basket.CustomerID, &basket.PaymentID, &items, &status)
	if err != nil {
